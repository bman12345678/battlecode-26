...
public class Motion {
    ...
    public static boolean canReachYou(MapLocation c) throws GameActionException {
        boolean ans = true;
        MapLocation me = G.rc.getLocation();
        Direction dir = me.directionTo(c);
        if (G.rc.canSenseLocation(me.add(dir))) {
            if (isDirtOrWall(me.add(dir))) ans = false;
        }
        if (G.rc.canSenseLocation(c.add(c.directionTo(me)))) {
            if (isDirtOrWall(c.add(c.directionTo(me)))) ans = false;
        }
        return ans;
    }
