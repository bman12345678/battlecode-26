...
//            }
            ...
            babyRatMicro();
            int rand = G.rc.getID() % 1000;
            if (G.rc.getRawCheese() > 0) MotionCodeGen.bugnavTowardsExplore(king);
            else if (G.rc.isMovementReady() && (rand>=mineset.size()*100 || mineset.size()<1 || (!mineLogic()))) {
    ...
    private static boolean mineLogic() throws GameActionException {
        if (visitedMines.size()==Math.min(mineset.size(),4)) visitedMines = new HashSet<>();
    ...
    private static MapLocation mineScore() throws GameActionException {
        int bestScore = Integer.MAX_VALUE; MapLocation bestMine = null;
        for (MapLocation e : mineset) {
            if (visitedMines.contains(e)) continue;
            int score = G.me.distanceSquaredTo(e) + king.distanceSquaredTo(e);
            if (score<bestScore) {
                bestScore = score; bestMine = e;
            }
        }
...
//               Motion.bugnavTowards(gatherPoint);
    ...
    public static boolean twoDisadvantadge(RobotInfo babyRat) throws Exception {
        ...
        else if(babyRat.getLocation().distanceSquaredTo(G.rc.getLocation()) == 5) newDist = 10;
        else newDist = 13;
