...
public class Motion {
    ...
    public static boolean canReachYou(MapLocation target) throws GameActionException {
        if (target == null) return false;
        if (!G.rc.onTheMap(target)) return false;
        if (!G.rc.canSenseLocation(target)) return false; // only sensed area

        MapLocation start = G.rc.getLocation();
        if (start.equals(target)) return true;

        MapInfo[] mps = G.rc.senseNearbyMapInfos(-1);
        int m = mps.length;

        // find bounding box of all sensed tiles + start + target
        int minX = Integer.MAX_VALUE, minY = Integer.MAX_VALUE;
        int maxX = Integer.MIN_VALUE, maxY = Integer.MIN_VALUE;
        for (int i = 0; i < m; i++) {
            MapLocation l = mps[i].getMapLocation();
            int lx = l.x, ly = l.y;
            if (lx < minX) minX = lx;
            if (ly < minY) minY = ly;
            if (lx > maxX) maxX = lx;
        ...
        // include start/target in box (they are sensed by checks above)
        if (G.rc.canSenseLocation(start)) {
        ...
        // present: 0 = not sensed, 1 = sensed
        // passable: 0 = not passable, 1 = passable
        byte[] present = new byte[area];
        byte[] pass = new byte[area];

        // fill from sensed MapInfo[]
        for (int i = 0; i < m; i++) {
            MapInfo mi = mps[i];
            MapLocation loc = mi.getMapLocation();
            int ix = (loc.x - minX) * height + (loc.y - minY);
            present[ix] = 1;
            pass[ix] = mi.isPassable() ? (byte)1 : (byte)0;
        }

        // ensure start is included (if sensed)
        int sidx = -1;
        if (G.rc.canSenseLocation(start)) {
            int ix = (start.x - minX) * height + (start.y - minY);
            if (present[ix] == 0) {
                MapInfo sm = G.rc.senseMapInfo(start);
                present[ix] = 1;
                pass[ix] = sm.isPassable() ? (byte)1 : (byte)0;
            }
            sidx = ix;
        } else {
            // if start isn't sensed we cannot reliably BFS in sensed-only area
            return false;
        }

        // ensure target is included (we already checked canSenseLocation)
        int tidx = -1;
        {
            int ix = (target.x - minX) * height + (target.y - minY);
            if (present[ix] == 0) {
                MapInfo tm = G.rc.senseMapInfo(target);
                present[ix] = 1;
                pass[ix] = tm.isPassable() ? (byte)1 : (byte)0;
            }
            tidx = ix;
        }

        // quick reject: if target not passable and not start -> unreachable
        if (tidx != sidx && pass[tidx] == 0) return false;

        // BFS arrays
        boolean[] vis = new boolean[area];
        int[] q = new int[area];
        int qh = 0, qt = 0;
        ...
        // neighbor deltas (8 directions)
        final int[] dx = {0,1,0,-1,1,-1,1,-1};
        final int[] dy = {1,0,-1,0,1,1,-1,-1};

        while (qh < qt && Clock.getBytecodesLeft()>6000) {
            int cur = q[qh++];
            if (cur == tidx) return true;

            // decode cur -> (cx,cy)
            int cxIndex = cur / height;       // (x - minX)
            int cyIndex = cur - cxIndex * height; // (y - minY)  (faster than %)
            int cx = minX + cxIndex;
            int cy = minY + cyIndex;
            ...
            for (int k = 0; k < 8; k++) {
                ...
                int ny = cy + dy[k];
                int nxIndex = nx - minX;
                if (nxIndex < 0 || nxIndex >= width) continue;
                int nyIndex = ny - minY;
                if (nyIndex < 0 || nyIndex >= height) continue;
                int nidx = nxIndex * height + nyIndex;
                if (present[nidx] == 0) continue;    // not sensed
                if (vis[nidx]) continue;
                if (pass[nidx] == 0) continue;       // not passable
                vis[nidx] = true;
                if (nidx == tidx) return true;
                q[qt++] = nidx;
            }
        }

        MapLocation c = target;
        boolean ans = true;
        MapLocation me = G.rc.getLocation();
