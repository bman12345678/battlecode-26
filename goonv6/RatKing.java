...
public class RatKing {
    ...
    private static MapLocation lastLoc = null;
    ...
    public static void run() throws Exception {
        spawned = false;
        if(lastLoc == null) lastLoc = new MapLocation(G.rc.getMapWidth()/2, G.rc.getMapHeight()/2);
        ...
        if(!seesMine) {
            for (RobotInfo r : G.rc.senseNearbyRobots(-1)) {
                if (r.getType().isBabyRatType() && r.getTeam() != G.opponentTeam && r.getRawCheeseAmount() > 0) {
                    lastLoc = r.getLocation();
                }
            }

            if(lastLoc != null) Motion.bugnavTowards(lastLoc);
        }
        ...
        if(G.rc.getCurrentRatCost() <= 100) {
            ...
            else if (G.rc.getGlobalCheese() - G.rc.getCurrentRatCost() + Math.min(1350, 4 * G.rc.getRoundNum()) >= 2350) {
                    spawnRats();
