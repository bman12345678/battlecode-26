    ...
    public static void run() throws Exception {
        ...
        for(MapInfo info : mapInfos) {
            if(info.hasCheeseMine()) {
                if(lastMine == null && !info.getMapLocation().isWithinDistanceSquared(king, G.rc.getMapWidth() * G.rc.getMapHeight()/16)) lastMine = info.getMapLocation();

               /*
               if(!info.getMapLocation().isWithinDistanceSquared(king, G.rc.getMapWidth() * G.rc.getMapHeight()/20)) {
        ...
        if(catTurnsLeft > 0 && catLoc != null) {
            catTurnsLeft -= 1;
            babyRatMicro();
            if(G.rc.getCurrentRatCost() >= 60) Motion.bugnavAwayNoTurning(catLoc);
        ...
        if(G.rc.getRoundNum() >= 1940 || ((G.rc.getID() % 7) > 5 && G.rc.readSharedArray(63) == 1) || (G.rc.getID() % 20 == 0 && G.rc.getCurrentRatCost() >= 80)) {
        ...
        if(cats.length > 0) {
            ...
            //if(Motion.canCatReachYou(cat.getLocation())) {
                ...
                catTurnsLeft = 1;
                babyRatMicro();
                if(G.rc.getCurrentRatCost() >= 60) Motion.bugnavAwayNoTurning(cat.getLocation());
        ...
        if(babyRat != null) {
            ...
            //(2, 2)
                ...
                Motion.bugnavTowardsNoTurning(babyRat.getLocation());
                Motion.turn(G.me.directionTo(babyRat.getLocation()));
            ...
            */
            ...
            if (G.rc.getCurrentRatCost() > 80 && G.rc.getGlobalCheese() > 1200 && G.rc.getRoundNum() < 1200) {

                if (enemyRobots.length == 0)
                    G.rc.squeak(16384 * (1024 * (1024 * G.rc.getLocation().x + G.rc.getLocation().y) + G.rc.getID()) + 1);
                ...
                for (Message m : squeaks) {
                    ...
                    if(lastMine == null) {
                        if(toGoTo != null) Motion.bugnavTowards(toGoTo);
                    }
                    else {
    ...
    public static void babyRatMicro() throws Exception {
        ...
        for(Direction d : Direction.allDirections()) {
            if(G.rc.canAttack(G.rc.getLocation().add(d))) {
                ...
                int cheeseToUse = 0;
                if(there.getHealth() == G.rc.getHealth() || there.getHealth() + 1 == G.rc.getHealth()) cheeseToUse = Math.min(G.rc.getGlobalCheese(), 1);
