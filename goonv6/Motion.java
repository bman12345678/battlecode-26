...
public class Motion {
    ...
    public static void bugnavTowardsExplore(MapLocation dest)  throws Exception{
        ...
        if (!G.rc.isMovementReady()) {
            if (G.rc.canTurn() && facing != desired) {
        ...
        if (facing != desired) {
            ...
            if (G.rc.canTurn() && G.rc.canMove(desired)) {
                try {
    ...
    public static boolean canCatReachYou(MapLocation c) throws Exception {
        Direction d = getBfsDirection(c);
        if(d == Direction.CENTER) {
            return false;
