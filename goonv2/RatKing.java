...
public class RatKing {
    ...
    public static void run() throws Exception {
        ...
        Message[] squeaks = G.rc.readSqueaks(G.rc.getRoundNum());
        for(Message m : squeaks) {
            System.out.println(m);
            int value = m.getBytes();
            int  x= value/1024;
            int y = value % 1024;
            MapLocation lol = new MapLocation(x, y);
            mines.add(lol);
        }
        int cnt = 0;
        int total = mines.size();
        G.rc.writeSharedArray(2, total);
        for(MapLocation m : mines) {
            G.rc.writeSharedArray(3 + cnt, m.x);
            G.rc.writeSharedArray(4 + cnt, m.y);
            cnt += 2;
        }
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type == UnitType.CAT || enemy.team == G.opponentTeam) {
                ...
                if(enemy.team == G.opponentTeam) {
                    G.rc.writeSharedArray(2, 1);
                }
            }
        }

        if(nearbyEnemies.length == 0) {
            G.rc.writeSharedArray(2, 0);
        }
        ...
        for(MapInfo m : G.rc.senseNearbyMapInfos()) {
            if(m.hasCheeseMine()) {
                seesMine = true;
            }
        }
        ...
        if (G.rc.getGlobalCheese() - G.rc.getCurrentRatCost() >= 800 + G.rc.getRoundNum()/6) {
            spawnRats();
        }
