...
public class Motion {
    ...
    public static MapLocation getClosest(MapLocation[] a) throws Exception {
        return getClosest(a, goonv4.G.rc.getLocation());
    }
    ...
    public static MapLocation getFarthest(MapLocation[] a) throws Exception {
        ...
        return getFarthest(a, goonv4.G.rc.getLocation());
    }
    ...
    public static void moveRandomly() throws Exception {
        if (goonv4.G.rc.isMovementReady()) {
            boolean stuck = true;
            for (int i = goonv4.G.DIRECTIONS.length; --i >= 0;) {
                if (goonv4.G.rc.canMove(goonv4.G.DIRECTIONS[i])) {
                    stuck = false;
                }
            }
            if (stuck) {
            ...
            // you came from
            Direction direction = goonv4.G.DIRECTIONS[goonv4.G.rng.nextInt(goonv4.G.DIRECTIONS.length)];
            if (direction == lastRandomDir.opposite() && goonv4.G.rc.canMove(direction.opposite())) {
    ...
    public static void spreadRandomly() throws Exception {
        boolean stuck = true;
        for (int i = goonv4.G.DIRECTIONS.length; --i >= 0;) {
            if (canMove(goonv4.G.DIRECTIONS[i])) {
                stuck = false;
            }
        }
        if (stuck) {
        ...
        if (goonv4.G.rc.isMovementReady()) {
            MapLocation me = goonv4.G.rc.getLocation();
            MapLocation target = me;
            for (int i = goonv4.G.allyRobots.length; --i >= 0;) {
                if (!goonv4.G.allyRobots[i].type.isRobotType())
                    // ignore towers
                    target = target.subtract(me.directionTo(goonv4.G.allyRobots[i].getLocation()));
            }
            for (int i = 7; --i >= 0;) {
                if (!goonv4.G.rc.canMove(goonv4.G.DIRECTIONS[i])) {
                    target = target.subtract(goonv4.G.DIRECTIONS[i]);
                }
            }
            if (target.equals(me)) {
                // just keep moving in the same direction as before if there's no robots nearby
                if (goonv4.G.rc.getRoundNum() % 3 == 0 || lastRandomSpread == null) {
                    moveRandomly(); // occasionally move randomly to avoid getting stuck
                } else if (goonv4.G.rng.nextInt(20) == 1) {
                    // don't get stuck in corners
                    lastRandomSpread = me.add(goonv4.G.DIRECTIONS[goonv4.G.rng.nextInt(goonv4.G.DIRECTIONS.length)]);
                    moveRandomly();
                } else {
    ...
    public static int[] simulateMovement(MapLocation me, MapLocation dest) throws Exception {
        MapLocation clockwiseLoc = goonv4.G.rc.getLocation();
        Direction clockwiseLastDir = lastDir;
        int clockwiseStuck = 0;
        MapLocation counterClockwiseLoc = goonv4.G.rc.getLocation();
        ...
        search: for (int t = 0; t < 10; t++) {
            ...
            Direction clockwiseDir = clockwiseLoc.directionTo(dest);
            {
                for (int i = 9; --i >= 0;) {
                    ...
                    if (goonv4.G.rc.onTheMap(loc)) {
                        if (!goonv4.G.rc.canSenseLocation(loc)) {
                            break search;
                        }
                        if (clockwiseDir != clockwiseLastDir.opposite() && goonv4.G.rc.senseMapInfo(loc).isPassable()
                                && goonv4.G.rc.senseRobotAtLocation(loc) == null) {
                ...
                for (int i = 9; --i >= 0;) {
                    ...
                    if (goonv4.G.rc.onTheMap(loc)) {
                        ...
                        if (counterClockwiseDir != counterClockwiseLastDir.opposite()
                                && goonv4.G.rc.senseMapInfo(loc).isPassable() && goonv4.G.rc.senseRobotAtLocation(loc) == null) {
    ...
    public static Direction bug2Helper(MapLocation me, MapLocation dest, int mode, int minRadiusSquared, int maxRadiusSquared) throws Exception {
        ...
        if (optimalDir != Direction.CENTER && mode != AROUND) {
            if (goonv4.G.rc.canMove(optimalDir) && lastDir != optimalDir.opposite()) {
                optimalDir = Direction.CENTER;
                rotation = NONE;
                visitedList = new StringBuilder();
            } else {
        ...
        if (lastDir != direction.opposite()) {
            if (goonv4.G.rc.canMove(direction)) {
                // if (!lastBlocked) {
                // rotation = NONE;
                // }
                // lastBlocked = false;
        ...
        } else if (goonv4.G.rc.canMove(direction)) {
            Direction dir;
            ...
            if (!goonv4.G.rc.onTheMap(me.add(dir))) {
                // boolean touchingTheWallBefore = false;
                // for (int i = DIRECTIONS.length; --i>=0;) {
                // MapLocation translatedMapLocation = me.add(d);
                // if (G.rc.onTheMap(translatedMapLocation)) {
        ...
        if (!goonv4.G.rc.onTheMap(me.add(direction))) {
            if (mode == AROUND) {
                circleDirection *= -1;
                direction = direction.opposite();
        ...
        for (int i = 8; --i >= 0;) {
            ...
            // return Direction.CENTER;
            // }
            if (goonv4.G.rc.canMove(direction) && lastDir != direction.opposite()) {
                if (flip) {
                    rotation *= -1;
                }
                if (goonv4.G.rc.canMove(direction)) {
        ...
        if (goonv4.G.rc.canMove(lastDir.opposite())) {
    ...
    public static void bugnavAway(MapLocation dest, Micro m) throws Exception {
        if (goonv4.G.rc.isMovementReady()) {
            Direction d = bug2Helper(goonv4.G.rc.getLocation(), dest, AWAY, 0, 0);
            if (d == Direction.CENTER) {
                d = goonv4.G.rc.getLocation().directionTo(dest);
            }
            m.micro(d, dest);
        }
    }
    ...
    public static void bugnavAround(MapLocation dest, int minRadiusSquared, int maxRadiusSquared, Micro m) throws Exception {
        if (goonv4.G.rc.isMovementReady()) {
            Direction d = bug2Helper(goonv4.G.rc.getLocation(), dest, AROUND, minRadiusSquared, maxRadiusSquared);
            if (d == Direction.CENTER) {
                d = goonv4.G.rc.getLocation().directionTo(dest);
            }
            m.micro(d, dest);
    ...
    public static void bfsInit() {
        width = goonv4.G.rc.getMapWidth();
        height = goonv4.G.rc.getMapHeight();
    ...
    public static void updateBfsMap() throws Exception {
        MapInfo[] map = goonv4.G.rc.senseNearbyMapInfos();
    ...
    public static void bfs() throws Exception {
        ...
        if (recalculationNeeded != MAX_PATH_LENGTH && recalculationNeeded < step) {
            ...
            step += 1;
            goonv4.G.indicatorString.append("BFS-RECALC ");
        }
        ...
        while (step < MAX_PATH_LENGTH && Clock.getBytecodesLeft() > 5000) {
            ...
            switch (height) {
                case 20:
                    goonv4.MotionCodeGen.bfs20();
                    break;
                case 21:
                    goonv4.MotionCodeGen.bfs21();
                    break;
                case 22:
                    goonv4.MotionCodeGen.bfs22();
                    break;
                case 23:
                    goonv4.MotionCodeGen.bfs23();
                    break;
                case 24:
                    goonv4.MotionCodeGen.bfs24();
                    break;
                case 25:
                    goonv4.MotionCodeGen.bfs25();
                    break;
                case 26:
                    goonv4.MotionCodeGen.bfs26();
                    break;
                case 27:
                    goonv4.MotionCodeGen.bfs27();
                    break;
                case 28:
                    goonv4.MotionCodeGen.bfs28();
                    break;
                case 29:
                    goonv4.MotionCodeGen.bfs29();
                    break;
                case 30:
                    goonv4.MotionCodeGen.bfs30();
                    break;
                case 31:
                    goonv4.MotionCodeGen.bfs31();
                    break;
                case 32:
                    goonv4.MotionCodeGen.bfs32();
                    break;
                case 33:
                    goonv4.MotionCodeGen.bfs33();
                    break;
                case 34:
                    goonv4.MotionCodeGen.bfs34();
                    break;
                case 35:
                    goonv4.MotionCodeGen.bfs35();
                    break;
                case 36:
                    goonv4.MotionCodeGen.bfs36();
                    break;
                case 37:
                    goonv4.MotionCodeGen.bfs37();
                    break;
                case 38:
                    goonv4.MotionCodeGen.bfs38();
                    break;
                case 39:
                    goonv4.MotionCodeGen.bfs39();
                    break;
                case 40:
                    goonv4.MotionCodeGen.bfs40();
                    break;
                case 41:
                    goonv4.MotionCodeGen.bfs41();
                    break;
                case 42:
                    goonv4.MotionCodeGen.bfs42();
                    break;
                case 43:
                    goonv4.MotionCodeGen.bfs43();
                    break;
                case 44:
                    goonv4.MotionCodeGen.bfs44();
                    break;
                case 45:
                    goonv4.MotionCodeGen.bfs45();
                    break;
                case 46:
                    goonv4.MotionCodeGen.bfs46();
                    break;
                case 47:
                    goonv4.MotionCodeGen.bfs47();
                    break;
                case 48:
                    goonv4.MotionCodeGen.bfs48();
                    break;
                case 49:
                    goonv4.MotionCodeGen.bfs49();
                    break;
                case 50:
                    goonv4.MotionCodeGen.bfs50();
                    break;
                case 51:
                    goonv4.MotionCodeGen.bfs51();
                    break;
                case 52:
                    goonv4.MotionCodeGen.bfs52();
                    break;
                case 53:
                    goonv4.MotionCodeGen.bfs53();
                    break;
                case 54:
                    goonv4.MotionCodeGen.bfs54();
                    break;
                case 55:
                    goonv4.MotionCodeGen.bfs55();
                    break;
                case 56:
                    goonv4.MotionCodeGen.bfs56();
                    break;
                case 57:
                    goonv4.MotionCodeGen.bfs57();
                    break;
                case 58:
                    goonv4.MotionCodeGen.bfs58();
                    break;
                case 59:
                    goonv4.MotionCodeGen.bfs59();
                    break;
                case 60:
                    MotionCodeGen.bfs60();
                    break;
        ...
        // }
        // }
        goonv4.G.indicatorString.append("BFS-STP=" + step + " ");
    }
    ...
    public static Direction getBfsDirection(MapLocation dest) throws Exception {
        ...
        if (optimalDirection == Direction.CENTER) {
            ...
            goonv4.G.indicatorString.append("BFS-BUG ");
    ...
    public static void bfsnav(MapLocation dest, Micro m) throws Exception {
        goonv4.G.indicatorString.append("BFS-BT: " + Clock.getBytecodesLeft() + "-");
        updateBfsTarget(dest);

        if (!goonv4.G.rc.getLocation().equals(dest) && goonv4.G.rc.isMovementReady()) {
        ...
        bfs();
        goonv4.G.indicatorString.append(Clock.getBytecodesLeft() + " ");
    }
    ...
    public static Micro defaultMicro = new Micro() {
        public void micro(Direction d, MapLocation dest) throws Exception {
            ...
            for (int i = 7; --i >= 0; ) {
                if (!goonv4.G.rc.canMove(goonv4.G.DIRECTIONS[i])) continue;
                int score = 0;
                MapLocation nxt = goonv4.G.me.add(goonv4.G.DIRECTIONS[i]);

                // Only sense if we can actually see that location
                if (goonv4.G.rc.canSenseLocation(nxt)) {
                    MapInfo info = goonv4.G.rc.senseMapInfo(nxt);
                    /*
                ...
                if (goonv4.G.DIRECTIONS[i] == d) {
                    score += 20;
                } else if (goonv4.G.DIRECTIONS[i].rotateLeft() == d || goonv4.G.DIRECTIONS[i].rotateRight() == d) {
                    score += 10;
                }
                if (score > bestScore) {
                    best = goonv4.G.DIRECTIONS[i];
                    bestScore = score;
                }
            }
            Motion.move(best);
